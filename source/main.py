...
bmp = BMP180(i2c)
bmp.oversample = 2
bmp.sealevel = 1009.45
sgp = Adafruit_SGP30(i2c)
 
wlan = network.WLAN(network.STA_IF)
wlan.active(True)
wlan.connect("Tvoje_WiFi_SSID","Tvoje_WiFi_Heslo")
 
def web_page():
    co2eq, tvoc = sgp.iaq_measure()
    temp = bmp.temperature
    press = round(bmp.pressure)
    alt = round(bmp.altitude)
    csv = open("meteo_data.csv","at")
    csv.write(f"{temp},{press},{alt},{co2eq},{tvoc}\n")
    csv.close()
    html = """<html lang="cz">
    <head>
        <meta charset="UTF-8">
        ...
        <style>
            ...
            table { border-collapse: collapse; width:55%; margin: 0 auto; }
            th { text-align: left; padding: 12px 30px; background-color: #87034F; color: white; }
            tr { text-align: left; border: 2px solid #000556; }
            tr:hover { background-color: #bcbcbc; }
            td { border: none; padding: 12px 30px; }
            .sensor { color:DarkBlue; font-weight: bold; background-color: none; padding: 1px;}
            img { display: block; margin: 20 auto; }  
        </style>
    </head>
    <body>
        <h1>Meteostanice - RPi PICO W</h1>
        ...
        <table>
            <tr><th>Typ</th><th>Hodnoty</th></tr>
            <tr><td>Teplota</td><td><span class="sensor">""" + str(temp) + """ °C</span></td></tr>
            <tr><td>Atm. tlak</td><td><span class="sensor">""" + str(press) + """ hPa</span></td></tr>
            <tr><td>Nadm. výška</td><td><span class="sensor">""" + str(alt) + """ m</span></td></tr>
            <tr><td>CO2</td><td><span class="sensor">""" + str(round(co2eq)) + """ ppm</span></td></tr>
            <tr><td>TVOC</td><td><span class="sensor">""" + str(round(tvoc)) + """ ppb</span></td></tr>
        </table> 
    </body>
</html>"""
    return html
...
while True:
    co2eq, tvoc = sgp.iaq_measure()
    if co2eq == 400:
        print("Inicializace modulu SGP30!")
        sleep(5)
    else:
        while True:
            try:
